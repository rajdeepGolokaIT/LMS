...
const AllDistributorSalesTable = () => {
    ...
    const filteredRecords = data.filter(distributors => {
        return (
          String(distributors.agencyName).toLowerCase().includes(searchTerm.toLowerCase()) ||
          String(distributors.contactPerson).toLowerCase().includes(searchTerm.toLowerCase())
        )
      });
