...
const Top5sold = () => {
  ...
  const fetchTopSellingProducts = async () => {
    try {
      ...
      if (selectedInterval === "Yearly") {
        apiUrl = `${BASE_URL}/api/v1/invoices/top-selling-products?interval=annually&year=${selectedYear}`;
        ...
        apiUrl = `${BASE_URL}/api/v1/invoices/top-selling-products?interval=${intervalParam}&fromDate=${fromDate}&toDate=${toDate}`;
      }
  ...
  const downloadPDF = () => {
    ...
    const title = "Top 5 Sold Products";
    const fontSize = 14;
    const textWidth =
    ...
    pdf.save("Top 5 Sold Products.pdf");
  };
    ...
    <TitleCard
      ...
      TopSideButtons2={
        ...
        <ul tabIndex={0} className="dropdown-content z-[1]  p-2 shadow bg-base-100 rounded-box">
          ...
          <li><button onClick={downloadPDF} className="btn btn-primary btn-sm mx-auto my-2 w-full">
            Download PDF
          </button></li>
          <li>
            <button onClick={handleReset} className="btn btn-bordered btn-sm mx-auto my-2 w-full">
            Reset
          </button></li>
        </ul>
      </div>
