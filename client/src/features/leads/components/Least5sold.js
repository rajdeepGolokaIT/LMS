...
export const Least5sold = () => {
  ...
  const fetchTopSellingProducts = async () => {
    try {
      ...
      if (selectedInterval === "Yearly") {
        apiUrl = `${BASE_URL}/api/v1/invoices/least-selling-products?interval=annually&year=${selectedYear}`;
      } else if (selectedInterval === "Monthly") {
        const monthYearArray = (selectedMonth.split(" "));
                const apiMonth = monthYearArray[0];
                const apiYear = monthYearArray[1];
        apiUrl = `${BASE_URL}/api/v1/invoices/least-selling-products?interval=monthly&year=${apiYear}&month=${apiMonth.toLowerCase()}`;
        ...
        apiUrl = `${BASE_URL}/api/v1/invoices/least-selling-products?interval=${intervalParam}&fromDate=${fromDate}&toDate=${toDate}`;
      }
  ...
  const downloadPDF = () => {
    ...
    const title = "Least 5 Sold Products";
    const fontSize = 14;
    const textWidth =
    ...
    pdf.save("Least 5 Sold Products.pdf");
  };
