  ...
  const fetchTopSellingProducts = async () => {
    try {
      ...
      if (selectedInterval === "Yearly") {
        apiUrl = `https://www.celltone.iskconbmv.org:8444/SalesAnalysisSystem-0.0.1-SNAPSHOT/api/v1/invoices/least-selling-products?interval=annually&year=${selectedYear}`;
      } else if (selectedInterval === "Monthly") {
        const apiMonth = selectedMonth.split(" ")[0];
        apiUrl = `https://www.celltone.iskconbmv.org:8444/SalesAnalysisSystem-0.0.1-SNAPSHOT/api/v1/invoices/least-selling-products?interval=monthly&year=${selectedYear}&month=${apiMonth.toLowerCase()}`;
        ...
        apiUrl = `https://www.celltone.iskconbmv.org:8444/SalesAnalysisSystem-0.0.1-SNAPSHOT/api/v1/invoices/least-selling-products?interval=${intervalParam}&fromDate=${fromDate}&toDate=${toDate}`;
      }
