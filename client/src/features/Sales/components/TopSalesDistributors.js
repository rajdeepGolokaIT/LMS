...
const TopSalesDistributors = () => {
    ...
    const fetchTopSellingProducts = async () => {
      try {
        ...
        if (selectedInterval === "Yearly") {
          apiUrl = `https://www.celltone.iskconbmv.org:8444/SalesAnalysisSystem-0.0.1-SNAPSHOT/api/v1/invoice/totalsales-of-top-distributors?interval=annually&${locationType}=${locationValue}&year=${selectedYear}&type=top`;
          console.log(apiUrl);
          //````````https://www.celltone.iskconbmv.org:8444/SalesAnalysisSystem-0.0.1-SNAPSHOT/api/v1/invoice/totalsales-of-top-distributors?interval=annually&zone=west&year=2024&type=top
        } else if (selectedInterval === "Monthly") {
            const monthYearArray = (selectedMonth.split(" "));
            const apiMonth = monthYearArray[0];
            const apiYear = monthYearArray[1].toString();
          apiUrl = `https://www.celltone.iskconbmv.org:8444/SalesAnalysisSystem-0.0.1-SNAPSHOT/api/v1/invoice/totalsales-of-top-distributors?interval=monthly&${locationType}=${locationValue}&year=${apiYear}&month=${apiMonth.toLowerCase()}&type=top`;
          console.log(apiUrl);
          ...
          apiUrl = `https://www.celltone.iskconbmv.org:8444/SalesAnalysisSystem-0.0.1-SNAPSHOT/api/v1/invoice/totalsales-of-top-distributors?interval=${intervalParam}&${locationType}=${locationValue}&customFromDate=${fromDate}&customToDate=${toDate}&type=top`;
          console.log(apiUrl);
  ...
  return (
    <>
    <TitleCard
       title="Top Sales by Distributors"
       topMargin="mt-2"
       TopSideButtons1={
         <>
           <select
...
export default TopSalesDistributors