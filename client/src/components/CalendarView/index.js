...
function CalendarView({calendarEvents, addNewEvent, openDayDetail}){
    ...
    const openAllEventsDetail = (date, theme) => {
        ...
        let filteredEvents = events.filter((e) => {return moment(date).isSame(moment(e.date), 'day') } ).map((e) => {return {id: e.id, title : e.title, date : e.date, theme : e.theme, content: e.content}})
